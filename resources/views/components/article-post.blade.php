...
<article class="flex flex-col sm:flex-row gap-4 sm:gap-2 group w-full">
    <div class="w-full sm:w-44 h-48 sm:h-28 rounded-xs overflow-hidden">
        <img src="/images/article-1.png" alt="{{ $post->title }}" class="group-hover:scale-110 transition-all duration-300 w-full h-full object-cover object-center">
    </div>
    <div class="space-y-2 w-full h-full group">
        <div class="inline-flex items-center text-sm/normal gap-2 font-bebas-neue dark:text-gray-200">
            <a id="categoryTag" href="/posts?category={{ $post->category->slug }}" class="category-tag px-2 py-1.5" style="--bg-category: {{ $post->category->color }}; --bg-category-hover:  @hexToRgba($post->category->color, 0.5);">
                {{ $post->category->name }}
            </a>
