...
<div class="bg-white dark:bg-slate-900 rounded-sm shadow p-4 md:p-6">
    ...
    <!-- Posts -->
    <div class="space-y-2">
        <div class="flex flex-col md:flex-row border-b py-2 border-gray-200 items-center justify-between group hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition duration-200 ease-in-out">
            ...
            <div class="flex w-full md:w-fit font-bebas-neue tracking-wider text-sm items-center space-x-2 mt-4 justify-between md:justify-items-start md:mt-0 ">
                <form method="POST" action="{{ route('posts.visibility', $post) }}" x-data="{ visibility: '{{ $post->visibility }}' }" class="flex items-center">
                    @csrf
                    @method('PATCH')

                    <input type="hidden" name="visibility" :value="visibility === 'public' ? '0' : '1'">

                    <button type="submit" @click.prevent="visibility = visibility === 'public' ? 'private' : 'public'; $el.form.submit();" class="relative overflow-hidden shadow-inner inline-flex items-center justify-center rounded-full w-20 h-8 text-sm font-medium transition-colors duration-300" :class="visibility === 'public' ?
                                    'bg-blue-100 text-blue-500' :
                                    'bg-orange-100 text-orange-600'">

                        <span class="absolute right-3 pt-0.5 transition-all duration-300 opacity-0" :class="visibility === 'public' ? 'opacity-100' : 'opacity-0'">Public</span>
                        <span class="absolute left-3 pt-0.5 transition-all duration-300 opacity-0" :class="visibility === 'private' ? 'opacity-100' : 'opacity-0'">Private</span>

                        <span class="absolute left-1/2 w-6 h-6 bg-white rounded-full shadow transform transition-transform duration-300" :class="visibility === 'public' ? '-translate-x-9' : 'translate-x-3'"></span>
                    </button>
                </form>
    ...
    <div class="flex flex-col items-center justify-center">
        ...
        <a href="/dashboard/posts/create" class="mt-2 font-bebas-neue tracking-wide px-4 py-2 bg-primary text-white rounded-sm shadow-md hover:bg-primary/80 transition duration-200 ease-in-out">
            Create Post
        </a>
    </div>
    @endforelse
