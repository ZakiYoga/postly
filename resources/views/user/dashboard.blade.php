...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <!-- Stat Card 4 -->
        <x-stat-card value="{{ $posts->sum('comments_count') }}" label="Total Comment" color="yellow"
        ...
        <div class="space-y-2">
            ...
            @else
                @foreach ($posts->take(4) as $post)
                    <!-- Posts -->
                    <div
                        class="flex border-b py-2 border-gray-200 dark:border-gray-500 rounded-xs w-full items-center flex-col md:flex-row justify-start md:justify-between group hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition duration-200 ease-in-out">
            ...
            @if ($posts->count() > 4)
                <div class="flex justify-center pt-3 mt-2">
                    <a href="/dashboard/posts"
                        class="inline-flex items-center font-medium px-4 py-2 text-sm text-gray-600 dark:text-gray-200 hover:text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:text-primary">
