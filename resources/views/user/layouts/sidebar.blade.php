<!-- Sidebar -->
<div x-cloak :class="sidebarOpen ? 'w-0 md:w-56' : 'w-20'" class="bg-white dark:bg-background-foreground/20 shadow-md pb-2 flex flex-col h-full overflow-hidden transition-all duration-300 overflow-x-hidden ">
    ...
    <!-- Menu Items -->
    <div :class="sidebarOpen ? 'opacity-0 md:opacity-100' : 'opacity-100'" class="my-6 overflow-y-auto overflow-x-hidden h-full custom-scrollbar font-bebas-neue tracking-wider">
        <nav class="px-2 pb-2 h-full">
            <!-- Create Post Menu -->
            @if (Auth::user()->role == 'user')
            <div class="mb-4" :class="sidebarOpen ?
                        'bg-primary/20 shadow-sm border border-gray-200 dark:border-gray-700' :
                        'bg-transparent shadow-none border-none mx-0'">
                <x-nav-link-sidebar href="/dashboard/posts/create" :active="request()->is('dashboard/posts/create')" class="flex items-center justify-center transition-all duration-150">
                    <span x-show="sidebarOpen" class="mr-4 leading-5 tracking-widest dark:text-white">Create <br>New Post</span>
                    <span class="inline-flex items-center justify-center border border-gray-200 dark:border-gray-700 bg-primary rounded-xs p-1">
            ...
            <!-- Dashboard Menu -->
            <x-nav-link-sidebar href="{{ Auth::user()->role == 'admin' ? '/admin/dashboard' : '/dashboard' }}" :active="Auth::user()->role == 'admin'
    ...
    <!-- Settings Menu -->
    <div :class="sidebarOpen ? 'opacity-0 md:opacity-100' : 'opacity-100'" class="mt-auto px-2 font-bebas-neue tracking-wider">
        <x-nav-link-sidebar href="{{ Auth::user()->role == 'admin' ? '/admin/dashboard/settings' : '/dashboard/settings' }}" :active="Auth::user()->role == 'admin'
