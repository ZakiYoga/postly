<x-layout>
    ...
    <section class="h-fit mt-2 px-4 sm:px-6 md:px-8 lg:px-16 w-full border rounded-xs border-gray-300 dark:border-gray-700 text-white bg-red-500">
            ...
            <div class="w-full lg:w-[60%] h-full mb-8 lg:mb-0" x-data="{ 
             ...
             ">
                ...
                <div class="relative w-full h-full overflow-hidden">
                    ...
                    <div class="relative w-full h-full">
                        <template x-for="(slide, index) in slides" :key="index">
                            <article class="absolute inset-0 w-full h-full transition-all duration-500 ease-in-out" :class="currentSlide === index ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'" x-show="currentSlide === index" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-x-full" x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 translate-x-0" x-transition:leave-end="opacity-0 -translate-x-full">
                                ...
                                <!-- Article Content -->
                                <div class="rounded-xs shadow-[0px_-8px_0px_#f5f5f5] drop-shadow-[4px_4px_0px_#f5f5f5] dark:shadow-[0px_-8px_0px_#030712] dark:drop-shadow-[4px_4px_0px_#030712] relative lg:absolute top-auto lg:top-0 lg:left-0 w-full lg:w-[45%] h-fit bg-background border border-gray-300 dark:border-gray-700 dark:bg-gray-900 z-10 space-y-2 p-4 mb-4 lg:mb-0">
                                    <div class="inline-flex items-center gap-2 font-bebas-neue text-sm/6 text-gray-800 dark:text-gray-200">
                                        <a href="#" class="tracking-wider p-0.5 px-1.5 dark:bg-[#050708]" x-text="slide.category || 'Gadget'"></a>
                                        <span class="w-5 h-[1px] bg-gray-300"></span>
                                        <p class="tracking-wider" x-text="slide.time_ago || '20 minute ago'"></p>
                                    </div>
                                    <p class="text-xl sm:text-2xl md:text-3xl font-semibold dark:text-white" x-text="slide.title || 'Lorem ipsum dolor sit amet, consectetur adipisicing elit.'">
                                    </p>
                                ...
                                <div class="lg:absolute lg:top-0 lg:right-0 w-full lg:pl-32 h-48 sm:h-64 md:h-80 lg:h-full">
                                    <img :src="slide.image || '/images/article-1.png'" :alt="slide.title || 'article'" class="rounded-xs w-full lg:aspect-[10/8] h-full object-cover object-center" />
                                </div>
                            </article>
                        </template>
                    </div>
                    ...
                    <!-- Navigation Buttons -->
                    <div class="flex items-center justify-between w-full mt-4 lg:justify-center lg:mt-0 lg:absolute z-20 lg:space-x-4 lg:bottom-4 lg:left-2 xl:left-2.5 lg:max-w-[15%] text-black dark:text-white">
                        <button @click="prevSlide()" class="w-10 h-10 p-1 bg-white dark:bg-gray-900 rounded-xs shadow-md transition-all duration-200 hover:shadow-lg hover:scale-105 active:scale-95">
                            <x-heroicon-m-chevron-left class="text-lg" />
                        </button>
                        <button @click="nextSlide()" class="w-10 h-10 p-1 bg-white dark:bg-gray-900 rounded-xs shadow-md transition-all duration-200 hover:shadow-lg hover:scale-105 active:scale-95">
        ...
        <div class="flex item-center p-8 justify-between gap-4 w-full h-full min-h-[80vh] bg-white/95 dark:bg-gray-900">
            <div class="flex flex-col w-1/3">
                ...
                <div class="flex flex-col gap-y-2 mt-4">
                    ...
                    @if ($index < 4) <article class="flex flex-col sm:flex-row py-2 rounded-xs gap-4 sm:gap-2 group w-full">
                        <div class="space-y-2 w-full h-full group">
                            <div class="inline-flex items-center text-sm/normal gap-2 font-bebas-neue">
                                <a href="#" class="tracking-wider p-0.5 px-1.5 text-[{{ $post->category->color }}] bg-[{{ $post->category->color }}]/15 group-hover:bg-[{{ $post->category->color }}] group-hover:text-white shadow-inner transition-all duration-500">{{ $post->category->name }}</a>
                            ...
                            <a href="/posts/{{ $post->slug }}" class="relative overflow-hidden inline-flex items-center gap-1 uppercase underline tracking-wider font-bebas-neue mt-auto dark:text-white hover:text-gray-900/80">
                                More
    ...
    <!-- Section Categories -->
    <section class="h-fit mt-8 px-4 sm:px-6 md:px-8 lg:px-16">
        <div class="flex item-center p-8 flex-col gap-4 w-full h-full min-h-[80vh] bg-white/95 dark:bg-gray-900 dark:text-gray-200">
            <div class="flex w-full justify-between items-center">
                ...
                <div>
                    <a href="/posts" class="font-bebas-neue hover:underline-offset-2 hover:underline hover:text-primary tracking-wide">View
                        All</a>
                </div>
            </div>

            <div class="relative w-full">
                <button class="prev absolute left-0 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-gray-800 p-2 rounded-full shadow-lg z-10 hover:bg-primary hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                    </svg>
                </button>

                <div class="slider-wrapper flex items-center min-h-[60px] relative w-full pointer-events-none overflow-hidden pb-4 px-10">
                    <div class="slider-inner absolute overflow-visible w-full h-full top-0 left-0 pb-1 flex gap-x-4 pointer-events-none transition-all duration-400 ease-out">
                        @foreach ($categories as $category)
                        <div class="slider-item flex items-center gap-2 p-2 border shadow-[2px_2px_0px_#000] flex-shrink-0 snap-start bg-white dark:bg-gray-800 hover:border-primary cursor-pointer transition-all">
                            @if ($category->image)
                            <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-8 h-8 object-cover">
                            @else
                ...
                <button class="next absolute right-0 top-1/2 -translate-y-1/2 bg-white/80 dark:bg-gray-800 p-2 rounded-full shadow-lg z-10 hover:bg-primary hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <article class="border bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded-lg">
                    @if ($post->thumbnail)
                    ...
                    <div class="p-4">
                        @if ($post->category)
                        <span class="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">{{ $post->category->name }}</span>
                        @endif
